...
- **视频导入**:从相册选择本地视频
- **视频预览**:VideoView 显示首帧
- **时间段选取**:Material RangeSlider 双 thumb 拖动,最小间隔 500ms
- **裁剪区域**:4 个 SeekBar(左/上/右/下各 0-50%),按百分比从对应边缘裁掉
...
2. 点"选择视频"从相册导入本地视频
3. 拖动 RangeSlider 两个 thumb 选时间段(最小间隔 500ms)
4. 调整 4 个 SeekBar(左/上/右/下)选裁剪区域,数字表示从该边缘裁掉多少百分比
